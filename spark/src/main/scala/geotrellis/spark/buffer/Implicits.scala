...
trait Implicits {
  ...
  implicit class withBufferTilesMethods(val self: BufferTiles.type) extends MethodExtensions[BufferTiles.type] {
      ...
      val sliced = layer
        .flatMap{ case (key, tile) => {
          ...
          def genSection(neighbor: layers.buffer.Direction): (K, (K, layers.buffer.Direction, V)) = {
            ...
            self.logger.debug(s"Generating buffer for $targetKey -> $targetDir from $key over span [${(l, t)}, ${(r, b)}]")
